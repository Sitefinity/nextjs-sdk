    ...
    private _getFormRulesFields () {
        ...
        for (let i = 0; i < this.formRules.length; i++) {
            ...
            for (let j = 0; j < this.formRules[i].actions.length; j++) {
                const getActionFieldIds = this.formRules[i].actions[j].executor.getActionFieldIds;
                if (getActionFieldIds){
                    let actionFieldIds = getActionFieldIds!(this.formRules[i].actions[j].data);
